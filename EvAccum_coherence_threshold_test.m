%% Find coherence threshold values for evidence accumulation task
% Dorian Minors
% Created: JUL19
% Last Edit: 13SEP19
...
%       parameters', below
%   currently only utilises 1 block, but can do more
...
p.training_coh_lvl_easy = 0.85; % what coherence level for easy (training level 1)
p.training_coh_lvl_mid = 0.60; % what coherence level for mid (training level 2)
p.training_coh_lvl_hard = 0.20; % what coherence level for hard (training level 3)
...
%       'dots.coherence' is specified within the trial loop
...
try
    ...
    for block = 1:p.act_block_num
        ...
        while i < p.act_trial_num
            ...
            %% present dots and collect response info (and initiate training protocol if specified)
            ...
            dots.coherence = p.stim_mat(i,6); % pulls coherence values from 'p.stim_mat' to give to moving_dots
            ...
            if p.training_enabled == 1 % if initiated
                ...
                elseif ~mod(i,p.training_interval) && p.training_dots_duration <= t.orig_dots_duration % if we've hit a trial that is divisible by the training interval and the current value of training dots duration is less than or equal to our final dots duration
                    ...
                    fprintf('dots duration now stable at p.dots_duration (%u secs) - now exposing to coherence changes\n', p.dots_duration);
                    if t.training_level == 0 && i >= 9
                        fprintf('training level %u at easy coherence - require %u (percent) correct to continue\n', t.training_level, p.training_percent_correct);
                        t.training_placeholder = p.training_coh_lvl_easy;
                    ...
                    if t.training_level == 1
                        fprintf('training level %u at easy coherence - require %u (percent) correct to continue\n', t.training_level, p.training_percent_correct);% fprintf('training level %u at mid coherence - exposure for %u trials\n', t.training_level, p.training_trials_per_level);
                        t.training_placeholder = p.training_coh_lvl_mid;
                        t.training_correct_counter = sum(d.correct(block,i-p.training_trials_per_level:i-1)); % movsum(d.correct,[p.training_trials_per_level 0]);
                    ...
                    if t.training_level == 2
                        fprintf('training level %u at hard coherence - exposure for %u trials\n', t.training_level, p.training_trials_per_level);
                        t.training_placeholder = p.training_coh_lvl_hard;
